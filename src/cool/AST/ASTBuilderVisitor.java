...
public class ASTBuilderVisitor extends CoolParserBaseVisitor<ASTNode> {
    ...
    public ASTNode visitClassDef(CoolParser.ClassDefContext ctx) {
        ...
        Token parentToken = (ctx.parent != null) ? ctx.parent : null;

        List<Feature> features = new ArrayList<>();
        for (var featureCtx : ctx.features) {
            features.add((Feature) visit(featureCtx));
        }
        Class classNode = new Class(className, inherits, parentToken, features, ctx.name);

        classNode.fileName = Compiler.fileNames.get(ctx);

        for (Feature feature : features) {
            feature.fileName = classNode.fileName;
        }
